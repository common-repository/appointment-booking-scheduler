...
$staff_id         = sanitize_text_field( $_REQUEST['staff_member'] );
$current_url      = sanitize_text_field( $_REQUEST['current_url'] );
$service_id       = sanitize_text_field( $_REQUEST['service'] );
$appointment_date = sanitize_text_field( $_REQUEST['apt_dates'] );
...
$selected_date        = sanitize_text_field( $_REQUEST['date_label'] );
...
foreach ( $staff_details as $staff_detail ) {
    ...
    <form style="margin-bottom: 0;" action="" method="POST" id="appoint_time" name="appoint_time">
        <div id="2" class="ap-steps-detail1">
            <p><?php _e( "Available time slots for", WL_ABS_SYSTEM ); ?> &nbsp; </p>
            <p class="service_tag">
				<?php
				...
				foreach ( $service_details as $service_detail ) {
					...
					if ( $service_duration_type == 'sd' ) {
						_e( $service_name, WL_ABS_SYSTEM );
						echo "&nbsp;(";
						_e( $service_duration . 'minutes', WL_ABS_SYSTEM );
						echo ")&nbsp;";
					}
				} ?>
            </p>
                    ...
                    <div class="swiper-wrapper ">
                        <div class="swiper-slide">
                            <div class="step-time">
                                <div class="aps-date">
                                    <label id="date_01"
                                           class="tm-value"><?php _e( $selected_date, WL_ABS_SYSTEM ); ?></label>
                                </div>
                                <ul class="stp-duration">
									...
									if ( $weekday == "Sunday" ) {
										...
										$lunch_start_time_staff = isset( $sunday_time[0]['break_start'] ) ? $sunday_time[0]['break_start'] : '' ;
										$lunch_end_time_staff   = isset( $sunday_time[0]['break_end'] ) ? $sunday_time[0]['break_end'] : '' ;
									...
									if ( $weekday == "Monday" ) {
										...
										$lunch_start_time_staff = isset( $monday_time[0]['break_start'] ) ? $monday_time[0]['break_start'] : '' ;
										$lunch_end_time_staff   = isset( $monday_time[0]['break_end'] ) ? $monday_time[0]['break_end'] : '' ;
									...
									if ( $weekday == "Tuesday" ) {
										...
										$lunch_start_time_staff = isset( $tuesday_time[0]['break_start'] ) ? $tuesday_time[0]['break_start'] : '' ;
										$lunch_end_time_staff   = isset( $tuesday_time[0]['break_end'] ) ? $tuesday_time[0]['break_end'] : '' ;
									...
									if ( $weekday == "Wednesday" ) {
										...
										$lunch_start_time_staff = isset( $wednesday_time[0]['break_start'] ) ? $wednesday_time[0]['break_start'] : '' ;
										$lunch_end_time_staff   = isset( $wednesday_time[0]['break_end'] ) ? $wednesday_time[0]['break_end'] : '' ;
									...
									if ( $weekday == "Thursday" ) {
										...
										$lunch_start_time_staff = isset( $thursday_time[0]['break_start'] ) ? $thursday_time[0]['break_start'] : '' ;
										$lunch_end_time_staff   = isset( $thursday_time[0]['break_end'] ) ? $thursday_time[0]['break_end'] : '' ;
									...
									if ( $weekday == "Friday" ) {
										...
										$lunch_start_time_staff = isset( $friday_time[0]['break_start'] ) ? $friday_time[0]['break_start'] : '' ;
										$lunch_end_time_staff   = isset( $friday_time[0]['break_end'] ) ? $friday_time[0]['break_end'] : '' ;
									...
									if ( $weekday == "Saturday" ) {
										...
										$lunch_start_time_staff = isset( $saturday_time[0]['break_start'] ) ? $saturday_time[0]['break_start'] : '' ;
										$lunch_end_time_staff   = isset( $saturday_time[0]['break_end'] ) ? $saturday_time[0]['break_end'] : '' ;
									...
									if ( $appointment_date == $current_date ) {
										...
										if ( $current_time < $end_slot_time ) {
											$slots           = array_filter( $total_time_slots, function ( $x ) use ( $current_time ) {
											...
											foreach ( $time_slots_available as $slot ) {
												...
												?>
                                                <li id="ap_slots" class="col-md-3 col-xs-6"><input class="radio_button"
                                                                                                   title="<?php echo $time_slot_format; ?>"
                                                                                                   value="<?php echo $slot; ?>"
                                                                                                   id="radio<?php echo $slot; ?>"
                                                                                                   type="radio"
                                                                                                   name="ap_time"/><label
                                                        for="radio<?php echo $slot; ?>" id="tm-value"
                                                        class="tm-value"><span
                                                            id="time_value<?php echo $slot; ?>"></span> <?php echo $time_slot_format; ?>
                                                </label></li><?php
											}
										} else {
											_e( "<center>Business Hours Closed for Today. Please Select Another Date.</center><br><center>Thank You</center>", WL_ABS_SYSTEM );
										}
									} else {
										...
										foreach ( $available_slots as $slot ) {
                                            ...
                                            <li id="ap_slots" class="col-md-3 col-xs-6"><input class="radio_button"
                                                                                               ...
                                                                                               type="radio"
                                                                                               name="ap_time"/><label
                                                    for="radio<?php echo $slot; ?>" id="tm-value" class="tm-value"><span
                                                        id="time_value<?php echo $slot; ?>"></span> <?php echo $time_slot_format; ?>
                                            </label></li><?php
										}
									}
        ...
        <div class="ap-step-link">
            ...
            <button id="stepback2" href="#step1" type="button" onclick="return step2_back();"
                    class="btn step-left"><?php if ( ! empty( $time_navigation_text_backward ) ) {
					_e( $time_navigation_text_backward, WL_ABS_SYSTEM );
				} else {
					_e( "Back", WL_ABS_SYSTEM );
				} ?></button>
            <button id="stepnext2" type="button" onclick="return step2_next();"
                    class="btn step-right"><?php if ( ! empty( $time_navigation_text_forward ) ) {
					_e( $time_navigation_text_forward, WL_ABS_SYSTEM );
				} else {
