...
<script>
    ...
    jQuery(document).on("click", '#step1_next', function (event) {
        ...
        if (appointment_service == 'default') {
            jQuery.notify("<?php _e( "Please Select Service", WL_ABS_SYSTEM ); ?>", {
                class: "alert_box",
                type: "danger",
                align: "center",
            ...
            jQuery.notify("<?php _e( "Please Select Staff", WL_ABS_SYSTEM ); ?>", {
                class: "alert_box",
                type: "danger",
                align: "center",
                verticalAlign: "middle"
            ...
            jQuery.notify("<?php _e( "Please Select date", WL_ABS_SYSTEM ); ?>", {
                class: "alert_box",
                type: "danger",
                align: "center",
                verticalAlign: "middle"
            ...
            jQuery('#step1_next').html('<i class="fa fa-spinner fa-spin"></i><?php _e( "Please Wait", WL_ABS_SYSTEM ); ?> ');
    ...
    function step2_next() {
        ...
        if (jQuery('.radio_button').is(':checked')) {
            ...
            jQuery('#stepnext2').html('<i class="fa fa-spinner fa-spin"></i> <?php _e( "Please Wait", WL_ABS_SYSTEM ); ?>');
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    jQuery('#stepnext2').html('<?php _e( "Next", WL_ABS_SYSTEM ); ?>');
            ...
            jQuery.notify("<?php _e( "Please Select Appointment Time", WL_ABS_SYSTEM ); ?>", {
                class: "alert_box",
                type: "danger",
                align: "center",
                verticalAlign: "middle"
    ...
    function step3_next() {
        ...
        if (login_button.clicked == true) {
            ...
            jQuery.notify("<?php _e( "Please Login Your Account", WL_ABS_SYSTEM ); ?>", {
                class: "alert_box",
                type: "danger",
                align: "center",
                verticalAlign: "middle"
    ...
    function step4_next() {
        ...
        jQuery.ajax({
            ...
            success: function (html) {
                ...
                jQuery('#stepnext4').html('<?php _e( "Save", WL_ABS_SYSTEM ); ?>');
...
if ( isset( $_REQUEST['appoint_staff_id'] ) ) {
	...
	$ap_staff_id              = sanitize_text_field( isset( $_REQUEST['appoint_staff_id'] ) ? $_REQUEST['appoint_staff_id'] : '0'  );
	...
	$temp_staff_member_name = $wpdb->get_col( "SELECT staff_member_name from $wpdb->prefix" . "apt_services  WHERE id='$ap_staff_id'" );
	$ap_staff_name          = isset( $temp_staff_member_name[0] ) ? $temp_staff_member_name[0] : '';
        ...
        <div id="ap_fornt_main_div" style="display:none;">
            <div class="row ap-logo">
                <div class="col-md-5 contact-info">
                    ...
                    <label id="business_name"><?php if ( $b_name == "" ) {
                            echo __( $blog_title, WL_ABS_SYSTEM );
                        } else {
                            echo __( $b_name, WL_ABS_SYSTEM );
                        } ?></label><br>
                    <?php
                    ...
                    ?>
                    <span class="bs_opening_hours"><i
                                class="fa fa-clock-o"></i> <?php
                        echo " " . $biz_start_time;
                        echo " - ";
                        echo $biz_end_time; ?></span><br>
                    <?php
                    ...
                    if ( $ap_show_phone_no == "yes" ) { ?>
                        <span class="phone_number_info"><i
                                    class="<?php echo $ap_phone_icon; ?>"></i> <?php echo $ap_phone_no; ?></span><br>
                    <?php }
                    ...
                    if ( $ap_show_email == "yes" ) { ?>
                        <span class="email_address_info"><i
                                    class="<?php echo $ap_email_icon; ?>"></i> <?php echo $ap_email; ?></span>
                        <?php
                    }
				...
				if ( $ap_show_logo == "yes" ) { ?>
                    <div class="col-md-2 wl_abs_logo_image">
						<?php
						$appearence_ap_logo = $wpdb->get_col( "SELECT ap_logo from $appearence_table" );
						$ap_logo            = $appearence_ap_logo[0];
						?>
                        <img src="<?php echo $ap_logo; ?>" class="img-responsive" alt=""/>
                    </div>
				<?php } ?>
                <div class="col-md-3 social-info col-md-offset-1">
                    <ul class="social">
						<?php
						...
						?>
						<?php if ( ! empty( $ap_social_link1 ) ) { ?>
                            <li><a href="<?php echo $ap_social_link1; ?>"><i
                                            class="<?php echo $ap_social_icon1; ?>"></i></a></li> <?php } ?>
						<?php if ( ! empty( $ap_social_link2 ) ) { ?>
                            <li><a href="<?php echo $ap_social_link2; ?>"><i
                                        class="<?php echo $ap_social_icon2; ?>"></i></a></li><?php } ?>
						<?php if ( ! empty( $ap_social_link3 ) ) { ?>
                            <li><a href="<?php echo $ap_social_link3; ?>"><i
                                        class="<?php echo $ap_social_icon3; ?>"></i></a></li><?php } ?>
						<?php if ( ! empty( $ap_social_link4 ) ) { ?>
                            <li><a href="<?php echo $ap_social_link4; ?>"><i
                                        class="<?php echo $ap_social_icon4; ?>"></i></a></li><?php } ?>
						<?php if ( ! empty( $ap_social_link5 ) ) { ?>
                            <li><a href="<?php echo $ap_social_link5; ?>"><i
                                        class="<?php echo $ap_social_icon5; ?>"></i></a></li><?php } ?>
                    </ul>
                </div>
            </div>
            ...
            <div id="step1">
                ...
                <form style="margin-bottom: 0;" action="" method="POST" id="step_01" name="step_01">
                    <div id="1" class="ap-steps-detail">
                        <div class="row service-form">
                            <div class="col-md-6 col-sm-6 ap-category">
                                <label><?php _e( "Select Service", WL_ABS_SYSTEM ); ?></label>
                                <select class="form-control " id="groups" name="service_name">
                                    <option value='default'>--<?php _e( "Select", WL_ABS_SYSTEM ); ?>--</option>
									<?php
									$settings_table            = $wpdb->prefix . "apt_settings";
									...
									foreach ( $appointment_category_details as $appointment_category_single_detail ) { ?>
                                        <optgroup label="<?php echo $appointment_category_single_detail->name; ?>">
											...
											foreach ( $appointment_details as $appointment_single_detail ) { ?>
                                                <option value="<?php echo $appointment_single_detail->id ?>"> <?php _e( $appointment_single_detail->name, WL_ABS_SYSTEM ); ?>
                                                    (<?php echo $appointment_single_detail->price ?><?php if ( $payment_currency == "" ) {
														echo ' USD';
													} else {
                            ...
                            <div class="col-md-6 col-sm-6 ap-dates">
                                <label><?php _e( 'Select Appointment Date', WL_ABS_SYSTEM ); ?></label>
						...
						if ( ! empty( $service_editor_content ) ) { ?>
                            <div class="row step-description">
                                <div> <?php echo "<pre class='pre_bg' style='display:block !important;'>" ?><?php _e( $service_editor_content, WL_ABS_SYSTEM ); ?><?php echo "</pre>" ?>  </div>
                            </div>
						<?php } ?>
                    </div>
                    <!-- Step 1 -->
                    <div class="ap-step-link">
                        ...
                        <button id="step1_next" type="button"
                                class="btn step-right"><?php if ( ! empty( $service_navigation_text ) ) {
								_e( $service_navigation_text, WL_ABS_SYSTEM );
							} else {
            ...
            <div id="step6" class="ap_class_active">
                ...
                <div id="6" class="ap-steps-detail6">
					...
					<?php } else { ?>
                        <h4 class="ap-heading"><?php _e( 'Booking Done', WL_ABS_SYSTEM ); ?></h4>
                        <div class="row service-form">
                            <p> <?php _e( "Thank You! Your Booking Is Complete.", WL_ABS_SYSTEM ); ?></p>
                        </div>
					<?php } ?>
                </div>
            </div>
