...
<script>
    ...
    function holiday_save() {
        ...
        jQuery('.save_appt_holiday').html('<i class="fa fa-spinner fa-spin"></i> <?php _e( "Creating", WL_ABS_SYSTEM ); ?>');
        ...
        jQuery.ajax({
            ...
            success: function (html) {
                ...
                jQuery('.save_appt_holiday').html('<?php _e( "Create Holiday", WL_ABS_SYSTEM ); ?>');
                jQuery.notify("<?php _e( 'Holiday Created Successfully', WL_ABS_SYSTEM ); ?>", {
                    type: "success",
                    icon: "check",
                    align: "center",
    ...
    function holiday_update() {
        ...
        jQuery('.update_appt_holiday').html('<i class="fa fa-spinner fa-spin"></i><?php _e( "Update", WL_ABS_SYSTEM ); ?>');
        jQuery.ajax({
            url: location.href,
            type: "POST",
            ...
            success: function (html) {
                ...
                jQuery('.update_appt_holiday').html('<?php _e( "Update Holiday", WL_ABS_SYSTEM ); ?>');
                jQuery.notify("<?php _e( 'Holiday Update Successfully', WL_ABS_SYSTEM ); ?>", {
                    type: "success",
                    icon: "check",
                    align: "center",
                    verticalAlign: "middle",
    ...
    jQuery(document).on("click", '.del_holi_day', function (event) {
        ...
        jQuery.confirm({
            ...
            theme: 'black',
            content: '<?php _e( "Are you sure to Delete Holiday?", WL_ABS_SYSTEM ); ?>',
            animation: 'rotate',
            ...
            confirm: function () {
                jQuery.ajax({
                    ...
                    type: "POST",
                    success: function (data) {
                        jQuery.notify("<?php _e( 'Delete Successfully', WL_ABS_SYSTEM ); ?>", {
                            type: "success",
                            icon: "check",
                            align: "center",
                            verticalAlign: "middle",
    ...
    jQuery(function () {
        jQuery("a.delete_holiday").click(function () {
            ...
            if (ids.length == 0) {
                jQuery.notify("<?php _e( 'Please Select At Least One Holiday To Delete', WL_ABS_SYSTEM ); ?>", {
                    type: "",
                    icon: "check",
                    align: "center",
                    verticalAlign: "middle",
                ...
                jQuery.confirm({
                    ...
                    confirm: function () {
                        ...
                        jQuery('.delete_holiday').html('<i class="fa fa-spinner fa-spin"></i><?php _e( "Deleting", WL_ABS_SYSTEM ); ?>');
                        ...
                        jQuery.ajax({
                            ...
                            success: function (res) {
                                ...
                                jQuery('.delete_holiday').html('<?php _e( "Delete", WL_ABS_SYSTEM ); ?>');
                                jQuery.notify("<?php _e( 'Holiday Delete Successfully', WL_ABS_SYSTEM ); ?>", {
                                    type: "success",
                                    icon: "check",
                                    align: "center",
                                    verticalAlign: "middle",
...
<?php
global $wpdb;
if ( isset( $_REQUEST['name'] ) && isset( $_POST['form_holiday_nonce'] ) ) {
	if ( ! wp_verify_nonce( $_POST['form_holiday_nonce'], 'form_holiday_nonce' ) ) {
		die();
	}
	if ( isset( $_REQUEST['allday'] ) ) {
		$allday = sanitize_text_field( $_REQUEST['allday'] );
	} else {
		$allday = "0";
	}
	...
	$wpdb->insert( $wpdb->prefix . 'apt_holidays', array(
		'all_off'         => $allday,
		'name'            => $name,
		...
		'repeat_value'    => $repeat,
		'repeat_days'     => $re_days,
		'repeat_weeks'    => $re_weeks,
		'repeat_bi_weeks' => $re_biweeks,
		'repeat_month'    => $re_months,
...
if ( isset( $_REQUEST['name_update'] ) && isset( $_POST['form_holiday_update_nonce'] ) ) {
	if ( ! wp_verify_nonce( $_POST['form_holiday_update_nonce'], 'form_holiday_update_nonce' ) ) {
		die();
	}
	if ( isset( $_REQUEST['allday_update'] ) ) {
	...
	$wpdb->update( $wpdb->prefix . 'apt_holidays', array(
		'all_off'         => $allday_update,
		'name'            => $name_update,
		'start_time'      => $start_time_update,
		'end_time'        => $end_time_update,
		'holiday_date'    => $event_date_update,
		'repeat_value'    => $repeat_update,
		'repeat_days'     => $re_days_update,
		'repeat_weeks'    => $re_weeks_update,
		'repeat_bi_weeks' => $re_biweeks_update,
		'repeat_month'    => $re_months_update,
		'start_date'      => $start_date_update,
		'end_date'        => $end_date_update,
		'notes'           => $note_update,
	), array( 'id' => $update_holiday_value ) );
	$wpdb->show_errors();
	$wpdb->print_error();
}
...
if ( isset( $_REQUEST['holiday_id'] ) ) {
	$del = sanitize_text_field( $_REQUEST['holiday_id'] );
...
<div class="panel panel-default">
    <div class="panel-heading"><i class="fa fa-coffee"></i><span
                class="panel_heading"><?php _e( 'Holiday', WL_ABS_SYSTEM ); ?></span>
        <div class="theme-export">
            <button type="button" class="btn theme-customer add-customer" data-toggle="modal" data-target="#holi_day"><i
                        class="fa fa-plus" aria-hidden="true"></i><?php _e( ' Add Holiday', WL_ABS_SYSTEM ); ?></button>
        </div>
    </div>
        ...
        <div class="theme-add-customer">
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title"><?php _e( 'Add Holiday', WL_ABS_SYSTEM ); ?></h4>
                        </div>
                        <div class="col-md-12 modal-body">
                            <div class="form-group">
                                <form action="" method="post" id="form_holiday">
									<?php wp_nonce_field( 'form_holiday_nonce', 'form_holiday_nonce' ); ?>
                                    <div class="row ad-ser">
                                        <div class="col-md-12 col-sm-12">
                                            <label><?php _e( 'All Day Off', WL_ABS_SYSTEM ); ?></label>
                                    ...
                                    <div class="row ad-ser">
                                        <div class="col-md-12 col-sm-12">
                                            <label><?php _e( 'Name', WL_ABS_SYSTEM ); ?></label>
                                            <input name="name" id="name" type="text" class="form-control"/>
                                            <span class="validation_alert"
                                                  id="holiday_name_alert"><?php _e( "This field is required", WL_ABS_SYSTEM ); ?></span>
                                        </div>
                                    </div>
                                    <div class="row ad-ser" id="day_off">
                                        <div class="col-md-6 col-sm-6">
                                            ...
                                            <div class="input-group clockpicker" data-placement="left" data-align="top"
                                                 data-autoclose="true">
                                                <input name="start_time" id="start_time_holiday" type="text"
                                                       class="form-control" value="<?php echo "10:00"; ?>"/>
                                            </div>
                                        </div>
                                        <div class="col-md-6 col-sm-6">
                                            ...
                                            <div class="input-group clockpicker" data-placement="left" data-align="top"
                                                 data-autoclose="true">
                                                <input name="end_time" id="end_time_holiday" type="text"
                                                       class="form-control"
                                                       value="<?php echo "10:30"; ?>"/>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row ad-ser p_date_hide">
                                        <div class="col-md-12 col-sm-12">
                                            ...
                                            <input name="event_date" id="event_date" type="text"
                                                   class="form-control holiday_dates"
                                                   value="<?php echo date( "m/d/Y" ); ?>"/>
                                        </div>
                                    </div>
                                    <div class="row ad-ser">
                                        <div class="col-md-12 col-sm-">
                                            <label><?php _e( 'Repeat', WL_ABS_SYSTEM ); ?></label>
                                            <select name="repeat" id="repeat" class="form-control"
                                                    onchange="holiday_function(this.value)">
                                                <option value="no"><?php _e( 'No', WL_ABS_SYSTEM ); ?></option>
                                                <option value="p_d"><?php _e( 'Particular Date(s)', WL_ABS_SYSTEM ); ?></option>
                                                <option value="daily"><?php _e( 'Daily', WL_ABS_SYSTEM ); ?></option>
                                                <option value="weekly"><?php _e( 'Weekly', WL_ABS_SYSTEM ); ?></option>
                                                <option value="bi_weekly"><?php _e( 'Bi-Weekly', WL_ABS_SYSTEM ); ?></option>
                                                <option value="monthly"><?php _e( 'Monthly', WL_ABS_SYSTEM ); ?></option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="row ad-ser hide_repeat_day all_hide" style="display:none">
                                        <div class="col-md-12 col-sm-12">
                                            <label><?php _e( 'Repeat Day(s)', WL_ABS_SYSTEM ); ?></label>
                                            <input name="re_days" id="re_days" type="number" value="0" maxlength="2"
                                                   class="form-control"/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row ad-ser hide_repeat_week all_hide" style="display:none">
                                        <div class="col-md-12 col-sm-12">
                                            <label><?php _e( 'Repeat Week(s)', WL_ABS_SYSTEM ); ?></label>
                                            <input name="re_weeks" id="re_weeks" type="number" value="0" maxlength="2"
                                                   class="form-control"/>
                                        </div>
                                    </div>
                                    <div class="row ad-ser hide_bi_repeat_week all_hide" style="display:none">
                                        <div class="col-md-12 col-sm-12">
                                            <label><?php _e( 'Repeat Bi-Week(s)', WL_ABS_SYSTEM ); ?></label>
                                            <input name="re_biweeks" id="re_biweeks" type="number" value="0"
                                                   maxlength="2" class="form-control"/>
                                        </div>
                                    </div>
                                    <div class="row ad-ser hide_repeat_month all_hide" style="display:none">
                                        <div class="col-md-12 col-sm-12">
                                            <label><?php _e( 'Repeat Month(s)', WL_ABS_SYSTEM ); ?></label>
                                            <input name="re_months" id="re_months" type="number" value="0" maxlength="2"
                                                   class="form-control"/>
                                        </div>
                                    </div>
                                    <div class="row ad-ser all_hide p_date_show all_hide" style="display:none">
                                        <div class="col-md-6 col-sm-6">
                                            <label><?php _e( 'Start Date', WL_ABS_SYSTEM ); ?></label>
                                            <input name="start_date" id="start_date" type="text" maxlength="2"
                                                   class="form-control holiday_dates"
                                                   value="<?php echo date( "m/d/Y" ); ?>"/>
                                        </div>
                                        <div class="col-md-6 col-sm-6">
                                            <label><?php _e( 'End Date', WL_ABS_SYSTEM ); ?></label>
                                    ...
                                    <div class="col-md-12 col-sm-12">
                                        <label><?php _e( 'Note', WL_ABS_SYSTEM ); ?></label>
                                        <textarea name="note" id="note" size="7" class="form-control"></textarea>
                                    </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button name="create" id="create" class="btn btn-primary save_appt_holiday" type="button"
                                    onclick="return holiday_save();"><?php _e( 'Create Holiday', WL_ABS_SYSTEM ); ?></button>
            ...
            <div class="modal fade" id="holi_day_update" role="dialog">
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title"> <?php _e( 'Update Holiday', WL_ABS_SYSTEM ); ?></h4>
                        </div>
                        <div class="col-md-12 modal-body">
                            <div class="form-group" id="holiday_html_fetch"></div>
                        </div>
                        <div class="modal-footer">
                            <button name="create" id="create" class="btn btn-primary update_appt_holiday" type="button"
                                    onclick="return holiday_update();"><?php _e( 'Update Holiday', WL_ABS_SYSTEM ); ?></button>
                            <button type="button" class="btn btn-default"
                                    data-dismiss="modal"> <?php _e( 'Cancel', WL_ABS_SYSTEM ); ?></button>
                        </div>
                    </div>
        ...
        <div id="mydiv" class="table-responsive">
            <form method="post" id="holiday_del" name="holiday_del">
                <table id="display_holiday" class="display" cellspacing="0" width="100%">
                    <thead>
                    <tr>
                        <th style="padding: 10px 12px;" class="nosort"><input type="checkbox" name="selec_tall_holiday"
                                                                              id="selec_tall_holiday" value=""></th>
                        <th class="sh_ow"> <?php _e( 'Name', WL_ABS_SYSTEM ); ?></th>
                        <th><?php _e( 'Date', WL_ABS_SYSTEM ); ?></th>
                        <th><?php _e( 'Time', WL_ABS_SYSTEM ); ?></th>
                        <th><?php _e( 'Repeat', WL_ABS_SYSTEM ); ?></th>
                        <th><?php _e( 'Status', WL_ABS_SYSTEM ); ?></th>
                        <th class="nosort"> <?php _e( 'Action', WL_ABS_SYSTEM ); ?></th>
                    </tr>
                    </thead>
                </table>
                <a href="#" class="delete_holiday btn btn-link"> <i class="fa fa-trash-o"
